...
def numDecodings1(s: str) -> int:
    if len(s) == 0 or s[0] == "0":
        return 0
    OurDP = [(1 if i == 0 or i == 1 else 0) for i in range(len(s) + 1)]
    for index in range(2, len(OurDP)):
        OneDigit = int(s[index-1])
        TwoDigits = int(s[index-2:index])

        if OneDigit != 0:
            OurDP[index] += OurDP[index-1]

        if 10 <= TwoDigits <= 26:
            OurDP[index] += OurDP[index - 2]
